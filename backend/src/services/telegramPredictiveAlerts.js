...
// Currently available/maybe_available stations whose historical (weekday,
// hour) profile predicts availability will fall below DROP_THRESHOLD_PCT
// within the next DROP_HOURS_AHEAD hours - a single bulk aggregation across
// every candidate station, not one query each.
 ...
 * Whole-region, not limited to a watchlist - kept affordable by bulk-
 * aggregating the (typically much larger) up-station set in one query and
 * only doing per-station work for the (typically much smaller) down-station
 * set. Each qualifying station's relevant cooldown timestamp is updated as
 * part of the scan so the same ongoing prediction isn't re-alerted on the
 * next tick.
 */
async function scanRegion(region, { now = new Date(), tz = DEFAULT_TZ } = {}) {
  ...
  const upStations = current.filter((s) => s.status === 'available' || s.status === 'maybe_available');
  const downStations = current.filter((s) => s.status === 'not_available');
...
module.exports = { scanRegion, DROP_THRESHOLD_PCT, DROP_HOURS_AHEAD, RECOVERY_WINDOW_MS, COOLDOWN_MS };
