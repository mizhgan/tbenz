...
<template>
  <div>
    ...
    <p class="hint">
      Добавьте бота в личный чат, группу или супергруппу — чат появится здесь со статусом «ожидает
      настройки» сразу после добавления (для групп это не требует ни одного сообщения от
      участников: бот видит сам факт своего добавления). Уведомления в чат начинают приходить
      только после того, как администратор переведёт его в статус «активен» и настроит
      районы/события.
    </p>
...
[data-theme='dark'] .bot-status.warn {
  color: #fcd34d;
}
</style>
